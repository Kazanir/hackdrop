...
namespace Drupal\hackutils\Theme;

use Drupal\Core\Theme\ThemeManager;

class AsyncThemeManager extends ThemeManager {
  public async function asyncRender(string $hook, array<mixed> $variables) {
    ...
    if (isset($info['function'])) {
      ...
      if ($render_function == 'twig_render_template') {
        // If nothing has messed with us so far, use our async rendering function
        // so that we can await it.
        $render_function = 'hackutils_async_twig_render_template';
      }
      ...
      // If nothing has intervened, we can call our async copy of twig_render_template
      // and await it, which completes the async tree.
      if ($render_function == 'hackutils_async_twig_render_template') {
        $output = await hackutils_async_twig_render_template($template_file, $variables);
      }
      else {
        $output = $this->render($template_file, $variables);
      }
    }
