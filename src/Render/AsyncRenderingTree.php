...
trait AsyncRenderingTree {
  ...
  protected async function asyncDoRender(array<mixed> &$elements, bool $is_root_call = FALSE): Awaitable<mixed> {
    ...
    if ($theme_is_implemented && !isset($elements['#render_children'])) {
      // @TODO Hackutils: Figure out how to async this if necessary. Does Twig
      // need its own asyncRender function? Ugh.
    ...
    if ((!$theme_is_implemented || isset($elements['#render_children'])) && empty($elements['#children'])) {
      $child_results = await vm($children, async $key ==> await $this->asyncDoRender($elements[$key]));
      $elements['#children'] = implode('', $child_results);
    ...
    if (isset($elements['#theme_wrappers']) && !isset($elements['#render_children'])) {
      // @TODO Hackutils: Turn this into an async vector once the
      // theme->asyncRender thing is done.
